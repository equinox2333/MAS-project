...
const Calendar = (props) => {
  ...
  const renderDay = (day, id) => {
    ...
    return (<View style={style.current.dayContainer} key={id}>
      <Day {...dayProps} testID={`${testID}.day_${dateString}`} date={dateString}
           state={getState(day, currentMonth, props, isControlled)} marking={markedDates?.[dateString]}
           onPress={_onDayPress} onLongPress={onLongPressDay}/>
      {
        // if(markeTextObj?.[dateString] ){return (<Text>ok</Text>)}
        markeTextObj?.[dateString]?.map((item, index) => {
          return (
            <Text key={index} style={{textDecorationLine: item?.isComplete ? 'line-through' : ''}}>{item.title}</Text>)
        })
      }
    </View>);
  };
  const renderWeek = (days, id) => {
    ...
    return (<View style={{...style.current.week, width: windowWidth}} key={id} testID='renderWeek'>
      {week}
    </View>);
  };
  const renderMonth = () => {
    ...
    return <View style={{...style.current.monthView,}} testID="vvvv">
      <Text style={{display: 'block', width: '100vw'}}> {weeks}</Text>
    </View>;
  ...
  const renderHeader = () => {
    ...
    return (
      <HeaderComponent {...headerProps} testID={`${testID}.header`} style={headerStyle} ref={ref} month={currentMonth}
  ...
  return (<GestureComponent {...gestureProps}>
    <View style={[style.current.container, propsStyle]} testID={testID}
...
Calendar.propTypes = {
  ...CalendarHeader.propTypes, ...Day.propTypes,
